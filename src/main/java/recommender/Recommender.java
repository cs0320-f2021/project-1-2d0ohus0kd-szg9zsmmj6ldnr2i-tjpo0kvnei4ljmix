...
public class Recommender implements RecommenderInterface{
   ...
   * @return List of best students for the given studentID
   */
  @Override
  public List<Student> bestStudents(int studentID, int k) {
    ...
    List<Student> kdRecommendations = this.kdt.nearestNeighbors(k, target);
    //Get recommendations from Bloom Filter
    List<Student> bloomRecommendations = this.bloomfilterrec.getTopKRecommendations(target, k);
